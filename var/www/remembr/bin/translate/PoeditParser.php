...
class PoeditParser
{
    ...
    public function getJSON($filter = null)
    {
        ...
        {
            if ($s->value && (!$filter || $filter($s)))
                $str[$s->key] = $s->value;
        }
        return json_encode($str);
    }

    public function toJSON($outputDir, $filter = null)
    {
        $outputFilename = $outputDir . '/l10n.js';
        $str = $this->getJSON($filter);

        if (!file_exists($outputDir))
        {
            mkdir($outputDir, 0755, true);  // @TODO: is this secure?
        }
