...
function UrlMatcher(pattern) {
  ...
  while ((m = placeholder.exec(pattern))) {
	...
	if(m[0].slice(-1) === '?') optional[id] = true;

	if (optional[id] && segment.slice(-1) === '/')
	{
...
UrlMatcher.prototype.format = function (values) {
  var segments = this.segments, params = this.params, optional = this.optional;
  ...
  for (i=0; i<nPath; i++) {
	...
	if(optional[params[i]] && result.slice(-1) == '/' && segments[i+1].slice(0,1) == '/')
	{
      ...
      var transition = $state.transition = resolved.then(function () {
        ...
        for (l=fromPath.length-1; l>=keep; l--) {
          ...
          if (exiting.self.onExit) {
            $injector.invoke(exiting.self.onExit, exiting.self, exiting.locals ? exiting.locals.globals : null);
          }
          exiting.locals = null;
        }
  ...
  return {
    ...
    link: function(scope, element, attrs, uiSrefActive) {
      ...
      var update = function(newVal) {
		var base = stateContext(element) || $state.$current;
